...
static int swtpm_start(struct swtpm *self)
{
    ...
    if (self->profile_rules != NULL) {
        profile_rules = g_strdup_printf("rules=%s", self->profile_rules);
        argv = concat_arrays(argv, (gchar*[]){"--profile", profile_rules, NULL}, TRUE);
...
static void swtpm_init(struct swtpm *swtpm,
                       ...
                       gboolean is_tpm2, const gchar *profile_rules)
{
    ...
    swtpm->profile_rules = profile_rules;
                         ...
                         const gchar *profile_rules)
{
    ...
    swtpm_init(&swtpm2->swtpm, swtpm_exec_l, state_path, keyopts, logfile,
               fds_to_pass, n_fds_to_pass, TRUE, profile_rules);
